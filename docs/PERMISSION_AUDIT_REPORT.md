...
2. **تخصیص‌یافته به خود** (`assignedTo === userId`)
3. **همان دپارتمان** (`departmentId` مکالمه = `departmentId` کاربر)
4. **همان شعبه** (`branchId` مکالمه = `branchId` کاربر)
5. **مشتری قابل دسترسی** (مشتری در لیست `getAccessibleCustomerIds`)
6. **مشارکت قبلی** (کاربر حداقل یک پیام در این مکالمه فرستاده)
...
- **بقیه** → مشتریان مکالمات تخصیص‌یافته به خود + مکالمات دپارتمان/شعبه + مشتریانی که کاربر در مکالماتشان پیام فرستاده
