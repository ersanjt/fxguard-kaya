...
| ثبت‌نام خودخدمت | `backend/public/signup.html` + `js/signup.js` + `routes/tenants.js` + `lib/selfServeBranding.js` | `/signup` | فقط با `SELF_SERVE_SIGNUP=true` روی **app**؛ لوگوی عمومی FXGuard؛ بعد از ثبت‌نام کوکی نشست + هدایت به داشبورد؛ تا قبل از DNS وایلدکارد ورود `?panel=slug` |
