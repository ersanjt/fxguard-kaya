/**
 * RabbitMQ connection and incoming message queue consumer
 */
const amqp = require('amqplib');
const { processIncomingMessage } = require('../incomingMessage');
const { setRabbitChannel } = require('../autoMessages');
...
async function connectRabbitMQ({ io, redisClient, logger }) {
    try {
        const timeout = (ms) =>
            new Promise((_, rej) =>
                setTimeout(() => rej(new Error('RabbitMQ connect timeout')), ms)
            );
        ...
        rabbitChannel.consume('whatsapp_messages', async (msg) => {
            ...
            try {
                ...
                logger.error('RabbitMQ: invalid JSON in message', {
        ...
        rabbitChannel = null;
        try {
            setRabbitChannel(null);
        } catch (_) {}
        if (!process.env.USE_SQLITE) {
            setTimeout(
                () => connectRabbitMQ({ io, redisClient, logger }),
                5000
            );
        }
    }
